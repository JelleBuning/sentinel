...
public static class ServiceCollectionExtensions
{
    ...
    {
        services.AddStartupTask<TimeSyncModule>();
        return services;
    }

    public static IServiceCollection AddConsumers(this IServiceCollection services, HubConnection hubConnection)
    {
        services.AddConsumer<RestartDeviceModule, RestartDeviceMessage>(config => config.Connection = hubConnection);
        services.AddConsumer<RemoteAccessModule, RemoteAccessMessage>(config => config.Connection = hubConnection);
        services.AddConsumer<SecurityScanModule, SecurityScanMessage>(config => config.Connection = hubConnection);
    ...
    public static IServiceCollection AddScheduledModules(this IServiceCollection services)
    {
        ...
        services.AddScheduledTask<DeviceInformationModule>(scheduleConfig => scheduleConfig.Interval = TimeSpan.FromMinutes(5));
        services.AddScheduledTask<StorageInformationModule>(scheduleConfig => scheduleConfig.Interval = TimeSpan.FromMinutes(10));
        services.AddScheduledTask<SecurityInformationModule>(scheduleConfig => scheduleConfig.Interval = TimeSpan.FromMinutes(2.5));
        services.AddScheduledTask<SoftwareInformationModule>(scheduleConfig => scheduleConfig.Interval = TimeSpan.FromMinutes(30));
        return services;
    }
    
    public static IServiceCollection AddStartupTask<T>(this IServiceCollection services) where T : class, IStartupModule, IModule
    {
    ...
    public static IServiceCollection AddConsumer<TConsumer, TMessage>(this IServiceCollection services, Action<IConsumerConfig<TMessage>> consumerConfig) where TConsumer : class, IHostedService, IModule
    {
    ...
    public static IServiceCollection AddScheduledTask<T>(this IServiceCollection services, Action<IScheduledModuleConfig<T>> scheduledTaskConfig) where T : ScheduledModuleBase<T>, IModule
    {
