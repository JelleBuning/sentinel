...
public abstract class ScheduledModuleBase<T>(ILogger<T> logger, IScheduledModuleConfig<T> config, bool runImmediately = false) : IHostedService, IModule
{
    ...
    {
        logger.LogInformation("[{Name}] Executing", Name);
        try
        {
            await Execute(cancellationToken);
            logger.LogInformation("[{Name}] Executed successfully", Name);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "[{Name}] Failed with message: {Message}", Name, ex.Message);
        }
        finally
        {
    ...
    {
        logger.LogInformation("Stopping {Name} task", Name);
        _timer?.Stop();
        await Task.CompletedTask;
    }
}